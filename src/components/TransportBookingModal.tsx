...
export default function TransportBookingModal({ isOpen, onClose, schedule }: TransportBookingModalProps) {
  ...
  const [bookingResult, setBookingResult] = useState<{ success: boolean; booking?: any; error?: string } | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // First, try to book with the transport service
      ...
      if (result.success) {
        // If booking is successful, save to localStorage
        const bookingData = {
          bookingId: result.bookingId || `BK${Date.now()}`,
          scheduleId: schedule.id,
          transportNumber: schedule.transportNumber,
          ...
          travelDate: new Date().toISOString().split('T')[0], // Use today as travel date for demo
          ...
          seatNumber: Math.random() > 0.5 ? `${Math.floor(Math.random() * 50) + 1}${String.fromCharCode(65 + Math.floor(Math.random() * 5))}` : undefined
        }

        const savedBooking = bookingService.saveBooking(bookingData)
        setBookingResult({ success: true, booking: savedBooking })
      } else {
                ...
                <>
                  ...
                  <div className="bg-gray-50 p-4 rounded-md mb-4">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="font-semibold text-gray-900">{schedule.fromStation}</span>
                      <span className="font-semibold text-gray-900">{schedule.toStation}</span>
                    </div>
                    <div className="flex justify-between items-center mt-2">
                      <span className="text-sm text-gray-600">{schedule.departureTime}</span>
                      <span className="text-sm text-gray-600">{schedule.arrivalTime}</span>
                    </div>
                    <div className="text-center mt-2">
                      <span className="text-sm text-green-600 font-semibold">₹{schedule.fare}</span>
                    </div>
                  </div>
                  ...
                  {bookingResult.success ? (
                    <>
                      ...
                      <p className="text-gray-600 mb-2">
                        Your booking ID: <span className="font-mono font-bold">{bookingResult.booking.bookingId}</span>
                      </p>
                      {bookingResult.booking.seatNumber && (
                        <p className="text-gray-600 mb-2">
                          Seat: <span className="font-semibold">{bookingResult.booking.seatNumber}</span>
                        </p>
                      )}
                      <p className="text-sm text-gray-500 mb-4">
                        Details have been saved to your bookings.
                      </p>
                    </>
                  ) : (
                    <>
